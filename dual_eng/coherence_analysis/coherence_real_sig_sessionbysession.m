...
for sub=1:2:16
    ...
    for session = 1:4
        ...
        group{g,session} = ft_appenddata(cfg,a,b);        
    end
...
for i= 1:size(group,1)
    for session = 1:4        
        cfg           = [];
        cfg.demean                    = 'yes';
        group{i,session} = ft_preprocessing(cfg,group{i,session});
    end
end
...
for i= 1:size(group,1)
    for session = 1:4        
        a{i,session} = ft_freqanalysis(cfg, group{i,session});        
        cfg_conn           = [];
        cfg_conn.method    = 'coh';
        cfg_conn.channelcmb = cfg.channelcmb;
        A{i,session} = ft_connectivityanalysis(cfg_conn, a{i,session});
    end
end
coh_full = A;
...
save([project.paths.processedData '/coherence/coh_real_sig_session'],'coh_full','group');
