...
load([project.paths.processedData '/coherence/coh_prob_real_surrogate.mat'])
...
%%
sig_channel = {'F5';'F7';'FT7';'FC5'};
a = cellfun(@(x) x(3:end), ch_comb.all(:,1), 'un', 0);
b = cellfun(@(x) x(3:end), ch_comb.all(:,2), 'un', 0);
X = [a b];
Y = nchoosek(sig_channel,2);

c = 5;

A = find(ismember(X(:,1),Y(c,1)));
B = find(ismember(X(:,2),Y(c,2)));
seleted_comb = intersect(A,B);
%%
freqLimt=5;
...
suptitle([X{seleted_comb,1} X{seleted_comb,2}])
