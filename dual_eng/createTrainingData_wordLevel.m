...
%%
function D = createTrainingData_wordLevel(project)
    ...
    for session = 1:6
        ...
        for word = 1:word_length
            ...
            if(exist(filename))
                ...
                %                 D(i,3) = str2num(strrep(temp_f,[project.subjects.list{sub} project.session.list{session} '-'],''));
                
                %%
                % f0
%                 c = spCepstrum(x, fs);
%                 D(i,4) = spPitchCepstrum(c, fs);
%                 % formant
%                 [n,f,a,b]=findFormant(x,project.preprocessing.formant_lpc_order,project.preprocessing.formant_threshold,project.preprocessing.formantexpected);
%                 D(i,5:8) = f;
%                 % word duration
%                 D(i,9) = length(x)/fs;
%                 % intensity
%                 D(i,10) = sqrt(sum(x.*x)/length(x));
%                 %power
%                 [Ptotal,P,F]=filter_third_octaves_downsample(x,project.preprocessing.reference_L_S_P,project.fs,25,12500,8);
%                 D(i,11) = Ptotal;
%                 % loudness
%                 [N_entire,N_single] = loudness_1991(x, project.preprocessing.reference_L_S_P, project.fs, 0);
%                 D(i,12) = N_entire;
%                 % sharpness
%                 D(i,13) = sharpness_Fastl(N_single);
...
save([project.paths.processedData '/processed_data_word_level.mat'],'D','W','mfc','mfc_raw','mfc_full','mfc_full_raw','P','fileList_name');
