...
%     data_conv2{i} = z_score_transformation(data_conv2{i});
%     data_noCh2{i} = z_score_transformation(data_noCh2{i});
end
 
 
 

[CONV_tf,~,GA_tf_CONV] = compute_ERSP(data_conv2,[-0.5 1],50/1e3,0);
[NOCH_tf,~,GA_tf_NOCH] = compute_ERSP(data_noCh2,[-0.5 1],50/1e3,0);
 
 
 
within_subj_stat = within_subj_tf_cluster(GA_tf_CONV,GA_tf_NOCH,neighbours,[8 40],15, [0 0.5]);

 
 [pos,neg] = significant_cluster_time_freq_channel(within_subj_stat,0.06);
...
% 
% a = squeeze(mean(TFR_diff.powspctrm(:,[6:9],[4:20],[11:21]),2));
% a = squeeze(mean(a,1));
% a = smooth2a(a,1,1);
% figure;imagesc(a);
...
% X.powspctrm([6 12 13],4,[13:15],[15:17]) = 0;
...
cfg.style            = 'straight';
cfg.marker           = 'off';
cfg.highlightchannel   = find(ismember(X.label,{'F5' 'F7' 'FC5'}));
cfg.interactive        = 'no';
cfg.highlight          =  'off';
cfg.highlightsymbol    =  '.';
